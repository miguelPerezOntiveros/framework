	...
	<div id="footerDownContainer">
		<div id="footerDownBody">
			...
			<div class="container">
				<div class="row">
					<div class="col-12">
						<div class="col-12 can_have_callout bs-callout-left">
							<h2 id="title"></h2>
						</div>

						<div class="col-12 newInquiry" id='callout' class="can_have_callout bs-callout-left">
							<br>
							<button type="button" id="createBtn" onclick="handleCreate();" class="btn btn-primary"><i class="fas fa-plus"></i></button>
	...
	//			$label.text(files.length > 1 ? ($input.attr('data-multiple-caption') || '').replace( '{count}', files.length ) : files[ 0 ].name);
		...
		doTable = function(name, displayName, thenDoForm) {
			...
			$.get('crud_read.php?table=' + name, function(data){
				...
				if(data.error){
					$('#feedbackModalBody').html('<div class="alert alert-danger" role="alert"><i class="fas fa-exclamation-circle"></i></span>&nbsp;Error: '+data.error+'</div>');
					...
					success: function(data) {
						...
						if(response.error)
							if(response.error == 'login')
								...
								$('#feedbackModalBody').html('<div class="alert alert-danger" role="alert"><i class="fas fa-exclamation">&nbsp;Error: '+response.error+'</div>');
							else{
								$('#feedbackModalBody').html('<div class="alert alert-success" role="alert"><i class="fas fa-check-circle"></i>&nbsp;'+response.success+'</div>');								
								setTimeout(function(){
