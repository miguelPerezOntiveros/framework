## IMPORTANT TODOs
- projectName/admin/config.inc.php should query the config from the DB
	- maker_mike.project.c.php should not write the config to projectName/admin/config.inc.php
	- set_config.php should be a unique file for all projects, that reads the DB
	- Change project column on MM DB to JSON datatype https://dev.mysql.com/doc/refman/5.7/en/json.html 
...
- on project crator backend ext, don't add page and portlet tables to the maker_mike project (possibly no user and userTypes either? if not, then add those back to the project specific section of the sidebar)
- $ grep --include=\*.php 'TODO' -rn .
- I don't think I need a discovery endpoint on each project, do I? Nor crud services on each project.
	- should be matter of just deleting some lines on pre.sh
