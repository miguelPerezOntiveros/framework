<?php
	...
	foreach ($data as $key => $value) {
		...
		foreach (json_decode($data[$key][4]) as $file_in_theme) {
			...
			// $data[$key][3] .= substr($file_in_theme, 0, -strlen($file_name)).'<a href="../../'.$config['_name'].'/'.$value[2].'/'.$file_name.'">'.$file_name.'</a><br>';
			$data[$key][3] .= '<a href="../../'.$config['_name'].'/'.$value[2].'/'.$file_in_theme.'">'.$file_in_theme.'</a><br>';
		}
		unset($data[$key][4]);
	}
